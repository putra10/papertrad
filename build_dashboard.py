...
def render(events, demo=False):
    ...
    tiles = [("Bot equity", fmt_money(bot),
              f"{((bot / start - 1) * 100):+.2f}% since start" if start else "—",
              bot - start)]
    ...
    for o in reversed(orders[-30:]):
        ...
        rows.append(
            ...
            f'<td class="why">{(o.get("reasoning") or "").strip() or "—"}</td></tr>')
    ...
    if last_call:
        ...
        drows = "".join(
            ...
            f'<td class="why">{(d.get("reasoning") or "").strip() or "—"}</td></tr>'
            for d in calls)
...
<div class="wrap">
  ...
  <p class="stamp">Alpaca paper account · fake money · updated {when(
      latest.get('timestamp')) if latest else 'never'}</p>
